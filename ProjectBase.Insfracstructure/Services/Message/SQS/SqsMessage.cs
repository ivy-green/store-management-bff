...
namespace ProjectBase.Insfracstructure.Services.Message.SQS
{
    public class SqsMessage : ISqsMessage
    {
        ...
        public SqsMessage(IAmazonSQS sqs, AppSettingConfiguration appSetting)
        {
            _sqs = sqs;
        ...
        public async Task<string> CreateQueueAsync(string queueName)
        {
            ...
            {
                throw new NullException("AWS setting section is missing");
            }
        ...
        public async Task<List<Amazon.SQS.Model.Message>> ReceiveSQSMessage(
            string queueName,
            ...
            bool isRestore = false)
        {
            ...
            List<Amazon.SQS.Model.Message> resReturn = [];
            while (true)
            {
        ...
        public async Task RemoveSQSMessage(string queueName, Amazon.SQS.Model.Message message)
        {
            var queueUrl = await GetSqsByName(queueName);
        ...
        public async Task<Amazon.SQS.Model.Message?> ReceiveSingleSQSMessage(
            string queueName,
            CancellationToken cancelToken,
            bool isRestore = false)
        {
